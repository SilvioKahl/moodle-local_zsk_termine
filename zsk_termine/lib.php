...
define('LOCAL_TERMINE_VIEW_UPCOMING', 'upcoming');
define('LOCAL_TERMINE_VIEW_PAST', 'past');
 ...
 *
 * @param string $activeview LOCAL_TERMINE_VIEW_UPCOMING|LOCAL_TERMINE_VIEW_PAST
 * @param int $categoryid
 * @return array<string,mixed>
 */
function local_zsk_termine_get_view_tabs_template_data(
    string $activeview = LOCAL_TERMINE_VIEW_UPCOMING,
    int $categoryid = 0
): array {
    $params = [];
    if ($categoryid > 0) {
    ...
    return [
        ...
        'pasturl' => (new moodle_url('/local/zsk_termine/index.php', array_merge($params, [
            'view' => LOCAL_TERMINE_VIEW_PAST,
        ])))->out(false),
        'pastlabel' => get_string('past_events', 'local_zsk_termine'),
        'activeupcoming' => $activeview === LOCAL_TERMINE_VIEW_UPCOMING,
        'activepast' => $activeview === LOCAL_TERMINE_VIEW_PAST,
    ];
}
 ...
 * @return string
 */
function local_zsk_termine_render_view_tabs_html(string $activeview = LOCAL_TERMINE_VIEW_UPCOMING, int $categoryid = 0): string {
    global $OUTPUT;

    return $OUTPUT->render_from_template('local_zsk_termine/view_tabs', local_zsk_termine_get_view_tabs_template_data(
        $activeview,
        $categoryid
    ));
}
...
function local_zsk_termine_render_upcoming_block_html(): string {
    ...
    $totalupcoming = local_zsk_termine_count_upcoming_events(null, LOCAL_TERMINE_COURSE_ALL);
    $totalpast = local_zsk_termine_count_past_events(null, LOCAL_TERMINE_COURSE_ALL);
    if ($totalupcoming === 0 && $totalpast === 0) {
        return '';
    }

    $events = $totalupcoming > 0
        ? local_zsk_termine_get_upcoming_events(null, $previewcount, false, LOCAL_TERMINE_COURSE_ALL)
        : [];
...
function local_zsk_termine_render_block_html(array $events, int $totalcount, int $previewcount): string {
    ...
    $data = array_merge(
        local_zsk_termine_get_view_tabs_template_data(LOCAL_TERMINE_VIEW_UPCOMING),
        [
